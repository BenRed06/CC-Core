# crystalcoin RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, crystalcoin-qt (or crystalcoind) will look for a file named crystalcoin.conf in the crystalcoin data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.crystalcoin/crystalcoin.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/crystalcoin.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\crystalcoin\crystalcoin.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\crystalcoin\crystalcoin.conf |

Note: if running crystalcoin in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of crystalcoin Core version 2.0.0:
...
Start crystalcoin Core Daemon with option:
<table>
<tr> <td>crystalcoind</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: crystalcoin.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: crystalcoind.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the crystalcoin and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in crystalcoin/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in crystalcoin/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all crystalcoin specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for crystalcoin inputs (0-1, default: 1)
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | crystalcoin )
...
#### preparebudget "proposal-name" "url" payment-count block-start "crystalcoin-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "crystalcoin-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or crystalcoin.conf setting gen) It can also be set with the setgenerate call.
...
Return information about the given crystalcoin address.
...
Wallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same crystalcoin address.

#### backupwallet "destination"

Safely copies wallet.dat to destination, which can be a directory or a path with filename.

#### bip38decrypt "crystalcoinaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "crystalcoinaddress" "passphrase"

Encrypts a private key corresponding to 'crystalcoinaddress'.

#### dumpprivkey “crystalcoinaddress”

Reveals the private key corresponding to ‘crystalcoinaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current crystalcoin address for receiving payments to this account.
...
Returns a new crystalcoin address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new crystalcoin address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “crystalcoinaddress” ( minconf addlockconf )

Returns the total amount received by the given crystalcoinaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “crystalcoinprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “tocrystalcoinaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a crystalcoin address.
...
#### sendtoaddress “crystalcoinaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending crystalcoins
...
Usage:
  crystalcoin-tx [options] <hex-tx> [commands]  Update hex-encoded crystalcoin transaction
  crystalcoin-tx [options] -create [commands]   Create hex-encoded crystalcoin transaction
